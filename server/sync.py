...
async def websocket_endpoint(ws: WebSocket):
    ...
    Checks for valid session cookie before accepting connection.

    Args:
        ws: WebSocket connection instance.
    """
    # Check for session authentication via cookies
    # The session middleware will have already parsed the session cookie
    # We need to verify the user is authenticated before accepting
    cookies = ws.cookies
    if "bear_session" not in cookies:
        await ws.close(
            code=status.WS_1008_POLICY_VIOLATION, reason="Authentication required"
        )
        return

    # Accept the connection (session validation happens via middleware)
    await ws.accept()
    clients.add(ws)
    ...
    try:
        ...
        released = [
            obj_id for obj_id, lock in soft_locks.items() if lock["owner"] is ws
        ]
