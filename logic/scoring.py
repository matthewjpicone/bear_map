...
def compute_ideal_allocation(map_data: Dict, castles: List[Dict]) -> List[Dict]:
    ...
    # Define zones
    tiles_bear1 = sorted(
        walkable, key=lambda t: chebyshev_distance(t[0], t[1], bear1["x"], bear1["y"])
    )
    tiles_bear2 = sorted(
        walkable, key=lambda t: chebyshev_distance(t[0], t[1], bear2["x"], bear2["y"])
    )
    ...
    for c in sorted_castles:
        ...
        if pref == "bear 1":
            ideal_travel_time = chebyshev_distance(
                ideal_x, ideal_y, bear1["x"], bear1["y"]
            )
        elif pref == "bear 2":
            ideal_travel_time = chebyshev_distance(
                ideal_x, ideal_y, bear2["x"], bear2["y"]
            )
        else:
...
def compute_efficiency(map_data: Dict, castles: List[Dict]) -> List[Dict]:
    ...
    if regrets:
        Tscale = (
            sorted(regrets)[int(len(regrets) * 0.9)]
            if len(regrets) >= 10
            else max(regrets)
        )
    else:
            ...
            for j, cj in enumerate(group_sorted):
                ...
                if ci["actual_travel_time"] < cj["actual_travel_time"]:
                    ...
                    if rank_diff > 0:
                        ...
                        block += (
                            cj["actual_travel_time"] - ci["actual_travel_time"]
                        ) * sigmoid_val
    ...
    if block_values:
        block_p90 = (
            sorted(block_values)[int(len(block_values) * 0.9)]
            if len(block_values) >= 10
            else max(block_values)
        )
